	...
	struct cred	*cr)
{
 ...
 error3:
	if (ldevvp) {
		VOP_CLOSE(ldevvp, vfs_flags, L_TRUE, cr, NULL, noerr);
		binval(logdev);
		VN_RELE(ldevvp);
	}
 error2:
	if (rdevvp) {
		VOP_CLOSE(rdevvp, vfs_flags, L_TRUE, cr, NULL, noerr);
		binval(rtdev);
		VN_RELE(rdevvp);
	}
 error1:
	if (ddevvp) {
		VOP_CLOSE(ddevvp, vfs_flags, L_TRUE, cr, NULL, noerr);
		binval(ddev);
		VN_RELE(ddevvp);
	}
 error0:
	...
	vnode_t		*devvp)
{
	...
	brelse(bp);
	VOP_CLOSE(devvp, FREAD, L_TRUE, get_current_cred(), NULL, unused);
	return error;
}
