...
typedef struct xfs_sb
{
	...
	u_int32_t	xfsb_blocksize;	/* logical block size, bytes */
	/*
	 ...
	 */
	u_int32_t	xfsb_magic;	/* magic number == XFS_SB_MAGIC */
	...
	u_int16_t	xfsb_version;	/* header version == XFS_SB_VERSION */
	u_int16_t	xfsb_sectsize;	/* volume sector size, bytes */
	u_int16_t	xfsb_inodesize;	/* inode size, bytes */
	u_int16_t	xfsb_inopblock;	/* inodes per block */
	char		xfsb_fname[6];	/* file system name */
	char		xfsb_fpack[6];	/* file system pack name */
	u_int8_t	xfsb_blocklog;	/* log2 of xfs_blocksize */
	u_int8_t	xfsb_sectlog;	/* log2 of xfs_sectsize */
	u_int8_t	xfsb_smallfiles;/* set if small files in inodes */
	...
	u_int32_t	xfsb_frextents;	/* free realtime extents */
} xfs_sb_t;
