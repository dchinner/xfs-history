	...
	int			level)
{
	...
	if (rbno != NULLFSBLOCK) {
		...
		right = xfs_buf_to_bmbt_block(rbuf);
	...
	if (block->bb_numrecs == XFS_BMAP_BLOCK_IMAXRECS(level, cur)) {
		...
		if (block->bb_numrecs < XFS_BMAP_BLOCK_DMAXRECS(level, cur)) {
			...
			*cblock = *block;
			...
			xfs_btree_check_lptr(cur, cbno, level);
			*pp = cbno;
